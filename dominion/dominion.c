...
void adventurerFunction(struct gameState *state, int handPos) {
...
void ghFunction(struct gameState *state, int handPos) {
  int currentPlayer = whoseTurn(state);
  int nextPlayer = currentPlayer + 1;
  if(nextPlayer > (state->numPlayers - 1)) {
    nextPlayer = 0;
...
void embargoFunction(struct gameState *state, int handPos, int choice1) {
  int currentPlayer = whoseTurn(state);
  int nextPlayer = currentPlayer + 1;
  if(nextPlayer > (state->numPlayers - 1)) {
    nextPlayer = 0;
